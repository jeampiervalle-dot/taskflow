...
<head>
    ...
    @vite(['resources/css/app.css', 'resources/css/style2.css', 'resources/js/app.js'])
...
<div class="modal_overlay hidden" id="taskModal">
    <div class="task_modal">
        ...
        <form id="taskForm" method="POST" action="{{ route('tasks.store') }}">
            ...
            <button type="submit" class="primary-btn btn-edit btn-save-task">
                Guardar tarea
            </button>
        </form>
    </div>
...
@endphp
@if($autoNotif)
<div class="toast-overlay show" id="taskToastOverlay"></div>
<div class="toast_popup task-toast" id="taskToast" style="display: block;">
    <div class="toast_content">
        ...
        <div class="toast_actions">
            ...
            <button class="toast_close_btn" onclick="closeToast('taskToast')">
                Cerrar
            </button>
        </div>
    </div>
...
<div class="toast-overlay hidden" id="systemToastOverlay"></div>
<div class="toast_popup hidden" id="systemToast">
    <div class="toast_content">
        <div class="toast_header_row">
...
function closeToast(toastId) {
    ...
    const overlay = document.getElementById(toastId + 'Overlay');
    if (overlay) {
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const overlay = document.getElementById('systemToastOverlay');
    if (systemToast) {
...
<div class="cat-typing" id="catTyping">
    <div class="cat-bongo">
        <img class="cat-layer" src="{{ asset('img/bongocat/cat.png') }}" alt="Bongo Cat" loading="lazy">
        <img class="hand-layer" id="lhand" src="{{ asset('img/bongocat/lhand1.png') }}" alt="" loading="lazy">
        <img class="hand-layer" id="rhand" src="{{ asset('img/bongocat/rhand1.png') }}" alt="" loading="lazy">
    </div>
</div>

<script>
(function() {
    let cat, lhand, rhand, timer;

    function init() {
        cat = document.getElementById('catTyping');
    ...
    document.addEventListener('keydown', function() {
        ...
        frame = (frame + 1) % 4;
        if (lhand) lhand.src = frame < 2
            ? "{{ asset('img/bongocat/lhand1.png') }}"
            : "{{ asset('img/bongocat/lhand0.png') }}";
        if (rhand) rhand.src = frame % 2 === 0
            ? "{{ asset('img/bongocat/rhand1.png') }}"
            : "{{ asset('img/bongocat/rhand0.png') }}";
        clearTimeout(timer);
