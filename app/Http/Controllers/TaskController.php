...
/**
 * Sistema de notificaciones de TaskFlow.
 *
 * Reglas:
 *  - Event-driven: crear, actualizar, completar y eliminar tareas generan
 *    notificación inmediata (creada, actualizada, completada, eliminada).
 *  - State-driven: cuando una tarea entra a la ventana de 72h o se vence,
 *    se genera UNA sola notificación (pendiente o vencida). Si el usuario
 *    la descarta, no se vuelve a generar a menos que cambie la fecha.
 *  - El toast del dashboard se muestra SOLO en la primera visita después
 *    del login (controlado por sesión).
 */
class TaskController extends Controller
{
    public function index()
    {
        ...
        $lastEditedTask = Task::where('user_id', $userId)
            ->orderBy('updated_at', 'desc')
            ->first();
        ...
        if ($nextTask) {
            ...
            if (!session()->has('dashboard_visitado')) {
                ...
                session()->put('dashboard_visitado', true);
            }
        } else {
    ...
    public function destroyNotification($id)
    {
        ...
        return redirect()->route('dashboard')->with('success', 'Notificación eliminada correctamente');
    }

    /**
     * Sincroniza la notificación automática de una tarea según su estado actual.
     *  - pending (>72h): borra notificaciones pendiente/vencida
     *  - near  (≤72h):  crea "pendiente" si no existe y no fue descartada
     *  - overdue:       crea "vencida" si no existe y no fue descartada
     */
    ...
    /**
     * Crea o actualiza una notificación basada en user_id + task_id + type.
     *  - Si existe, solo refresca título y mensaje (no genera duplicado).
    ...
    /**
     * Limpia duplicados antiguos que no tengan task_id (de la versión previa del sistema).
     * Se ejecuta en cada visita al dashboard para mantener la bandeja ordenada.
     */
