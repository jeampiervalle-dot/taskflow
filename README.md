...
## 🌍 Despliegue con Cloudflare Tunnel (gratis)

> Recomendado para **probar** la app en Internet sin pagar hosting.
...
### Paso 2 — Autenticarse (una sola vez)

```powershell
cloudflared tunnel login
```
Esto abre el navegador para vincular tu cuenta de Cloudflare.
...
Crea `C:\Users\Admin\.cloudflared\config.yml`:

```yaml
tunnel: taskflow
credentials-file: C:\Users\Admin\.cloudflared\<TU_TUNNEL_ID>.json

ingress:
  - hostname: taskflow.tu-dominio.com
...
```powershell
cloudflared tunnel route dns taskflow taskflow.tu-dominio.com
```
...
# Terminal 2: Túnel
cloudflared tunnel run taskflow
```

Tu app quedará accesible en `https://taskflow.tu-dominio.com` 🎉
